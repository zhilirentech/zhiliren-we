...
<c:set var="url" value="${base}/admin/finance" />
...
<div class="panel_search"> 
	<form action="${url}/list.html" method="post" onsubmit="return navTabSearch(this);">
		<input type="hidden" name="pageSize" value="${obj.pager.pageSize}" />
		<input type="hidden" name="pageNumber" value="${obj.pager.pageNumber}" />
		<p><span class="tag">姓名：</span><span class="ipts"><input type="text" name="cName" value=""/></span></p>
			...
			<tbody>
				<c:forEach items="${obj.list}" var="one" >
					<tr>
						...
						<td>${one.money}</td>
						<td>
							<c:if test="${ one.checkStatus == 0}">
								未审核
							</c:if>
							<c:if test="${ one.checkStatus == 1}">
								通过
							</c:if>
							<c:if test="${ one.checkStatus == 2}">
								不通过
							</c:if>
						</td>
						<td>${one.checkInfo}</td>
						<td>
							<c:if test="${one.checkStatus==0}">
								<a target="dialog" rel="dlgId1" href="${url}/getWidthAppById.html?uId=${one.id}" class="btn btn_mini btn_modify">审核</a>
							</c:if>
						</td>
					</tr>
				</c:forEach>
