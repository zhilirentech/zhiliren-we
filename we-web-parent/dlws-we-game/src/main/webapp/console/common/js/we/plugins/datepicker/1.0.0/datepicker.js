define('datepicker',['we/plugins/datepicker/1.0.0/util.date.js','we/plugins/datepicker/1.0.0/style.css'],function(require, exports, module){
	/**
	...
	(function($){
		$.fn.datepicker = function(opts){
			var setting = {
				...
				close$:"#calendar .close", calIcon$:"a.dateicon",
			...
			function keydownInt(e){
				if (!((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode == we.keyCode.DELETE || e.keyCode == we.keyCode.BACKSPACE))) { return false; }
			}
			...
			return this.each(function(){
				...
				function generateCalendar(dp){
					...
					$days.not('.disabled').click(function(){
						...
						if (!dp.hasTime()) {
							$this.val(dp.formatDate(dp.changeDay($day.attr("day"), $day.attr("chMonth")))).focus();
							if(opts.changeMinDate){
								var _sub = $this.parents('form').find("input[name='"+opts.changeMinDate+"']");
								_sub.attr('minDate',$this.val()).val($this.val());
								we.datepicker(_sub);
								$this.change();
							}
							closeCalendar(); 
				...
				$this.click(function(event){
					...
					$(setting.tmpl).appendTo("body").css({
						left:offset.left+'px',
						top:iTop+'px'
					...
					var dayNames = "";
					$.each(setting.dayNames, function(i,v){
						dayNames += "<dt>" + v + "</dt>"
					});
					...
					$.each(setting.monthNames, function(i,v){
						var m = i+1;
