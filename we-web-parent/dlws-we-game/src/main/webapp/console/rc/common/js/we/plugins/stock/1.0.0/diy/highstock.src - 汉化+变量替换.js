...
// JSLint options:
/*global Highstock, document, window, navigator, setInterval, clearInterval, clearTimeout, setTimeout, location, jQuery, $, console, each, grep */
...
// The Highstock namespace
var Highstock = win.Highstock = win.Highstock ? error(16, true) : {};
 ...
 * First, it deep merged arrays, which lead to workarounds in Highstock. Second,
...
function error(code, stop) {
	var msg = 'Highstock error #' + code + ': www.highcharts.com/errors/' + code;
	if (stop) {
		throw msg;
	} else if (win.console) {
		console.log(msg);
...
(function ($) {
	/**
	 * The default HighstockAdapter for jQuery
	 */
	win.HighstockAdapter = win.HighstockAdapter || ($ && {
		...
		init: function (pathAnim) {
			...
			// extend some methods to check for elem.attr, which means it is a Highstock SVG object
			...
			/**
			 * Register Highstock as a plugin in the respective framework
			 */
			$.fn.highstock = function () {
				...
				if (options !== UNDEFINED) {
					...
					chart = new Highstock[constr](options, args[1]);
					ret = this;
					/*jslint unused:true*/
				}
		...
		fireEvent: function (el, type, eventArguments, defaultFunction) {
			...
			// Remove warnings in Chrome when accessing layerX and layerY. Although Highstock
...
// check for a custom HighstockAdapter defined prior to this file
var globalAdapter = win.HighstockAdapter,
...
// Utility functions. If the HighstockAdapter is not defined, adapter is an empty object
 ...
 */
Highstock.VMLElement = VMLElement = {
...
Highstock.VMLRenderer = VMLRenderer = function () {
	this.init.apply(this, arguments);
};
...
if (useCanVG) {
	 ...
	 */
	Highstock.CanVGRenderer = CanVGRenderer = function () {
 ...
 * @param {Object} options
 */
Highstock.PlotLineOrBand = function (axis, options) {
	this.axis = axis;
...
Highstock.PlotLineOrBand.prototype = {
...
AxisPlotLineOrBandExtension = {
	...
	addPlotBandOrLine: function (options, coll) {
		var obj = new Highstock.PlotLineOrBand(this, options).render(),
				...
				each(tickPositions, function (pos, i) {
					if (i % 2 === 0 && pos < axis.max) {
						if (!alternateBands[pos]) {
							alternateBands[pos] = new Highstock.PlotLineOrBand(axis);
						}
 ...
 */
var Tooltip = Highstock.Tooltip = function () {
	this.init.apply(this, arguments);
};
 ...
 */
var Pointer = Highstock.Pointer = function (chart, options) {
...
Pointer.prototype = {
	...
	init: function (chart, options) {
		...
		if (Highstock.Tooltip && options.tooltip.enabled) {
...
extend(Highstock.Pointer.prototype, {
 ...
 */
var Legend = Highstock.Legend = function (chart, options) {
	this.init(chart, options);
};
...
var LegendSymbolMixin = Highstock.LegendSymbolMixin = {
	 ...
	 * symbols through Highstock.seriesTypes[type].prototype.drawLegendSymbols.
	 * 
	 * @param {Object} legend The legend object
	 */
...
var CenteredSeriesMixin = Highstock.CenteredSeriesMixin = {
	/**
...
Series.prototype = {
		...
		if (updatePoints !== false && dataLength && oldDataLength === dataLength && !series.cropped && !series.hasGroupedData) {
			...
			if (turboThreshold && dataLength > turboThreshold) {
				...
				if (isNumber(firstPoint)) { // assume all points are numbers
					...
					error(12); // Highstock expects configs to be numbers or arrays in turbo mode
				}
			} else {
				for (i = 0; i < dataLength; i++) {
	 ...
	 * unused points and optionally grouped in Highstock Stock.
	 */
...
var TrackerMixin = Highstock.TrackerMixin = {
...
if (Renderer === Highstock.VMLRenderer) {
...
Highstock.RangeSelector = RangeSelector;
 ...
 */
Highstock.StockChart = function (options, callback) {
